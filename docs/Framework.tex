...
This section presents \sysname, a trace-guided AutoResearch framework for discovering EDA-native GNN designs beyond a fixed neural architecture search space. \sysname treats automated model design as a sequence of falsifiable \emph{EDA modeling hypotheses}, rather than as a sequence of unconstrained code edits. Semantic co-design provides the grounding coordinate system for these hypotheses, while the trace protocol makes their execution and evidence attributable. The framework realizes each hypothesis within a trace-local scope, validates its grounding and implementation together, executes it under a comparable research context, and records validation evidence as a design trace. Completed traces become structured research memory for subsequent exploration.
...
Figure~\ref{fig:framework} presents one HEDGE research cycle in three stages. \emph{Task and context} define the research boundary: an EDA task specifies the graph schema, prediction target, and domain constraints; these inputs instantiate a ResearchContext $\kappa$. Together with completed trace memory $\mathcal{M}_t=\{T_0,\ldots,T_t\}$, the context determines which parent traces are comparable and which quantities must remain fixed.

\emph{Trace-grounded exploration} turns this state into one controlled candidate. HEDGE forms a hypothesis card $H_i$ containing an EDA rationale, expected observation, and falsification condition, then realizes it in trace-local editable modules. A single role-separated Candidate Reviewer examines the authored candidate: it checks both the EDA grounding of $H_i$ and the alignment between $H_i$ and the exact parent-relative delta $\Delta_i$. Runtime protocol validation subsequently checks the editable scope, artifact completeness, context and lineage consistency, interface contracts, and review freshness before execution.

\emph{Controlled evaluation and trace feedback} separate execution from interpretation. The validated candidate is trained and selected under the fixed validation protocol. The resulting validation evidence $E_i$ and permitted diagnostics are analyzed as \texttt{supported}, \texttt{refuted}, or \texttt{inconclusive}, and are stored with the parent, hypothesis, delta, approved configuration, and analysis record as $T_i=(p_i,H_i,\Delta_i,C_i,E_i,\mathcal{A}_i)$. The trace returns to memory: supported mechanisms may be extended or ablated, refuted directions are pruned, and inconclusive traces motivate a narrower diagnostic or restart. Thus HEDGE separates \emph{what} to explore, \emph{how} it is implemented, and \emph{what} the evidence implies. Held-out evaluation, when reported, occurs only after search and cannot influence this loop.
...
\FOR{$t=0$ to $N-1$}
    ...
    \STATE Candidate Review of grounding and alignment between $H$ and $\Delta$
    \IF{candidate review or workspace validation fails}
        \STATE revise or discard the candidate; \textbf{continue}
    \ENDIF
    \STATE Execute the candidate under $\kappa$
    \STATE Analyze validation evidence; record outcome and next action
    \STATE Append $T=(T_p,H,\Delta,C,E,\mathcal{A})$ to $\mathcal{M}$
\ENDFOR
\end{algorithmic}
\end{algorithm}

\subsection{Semantic Hypothesis Formation}
...
Each trace begins with a structured hypothesis card
\begin{equation}
H_i=(\mathcal{G}_i,\mathcal{X}_i,S_i,P_i),
\label{eq:hypothesis_card}
\end{equation}
where $\mathcal{G}_i=(M_i,R_i,O_i,A_i,L_i)$ is the semantic grounding of the hypothesis: its task mechanism, modeling rationale, concrete EDA objects, analysis rule, and intended L1--L3 levels. $\mathcal{X}_i=(E_i,F_i,V_i)$ specifies its experimental claim through the expected observation, falsification condition, and controlled variables; $S_i$ declares the permitted implementation scope; and $P_i$ cites validation evidence inherited from the parent trace. A child card must cite its actual parent and may not use held-out observations to motivate a later hypothesis.
...
The L1--L3 labels are commitments rather than a closed library of operators. They state where a claimed mechanism must be realized: L1 in an enabled graph transformation, L2 in model or convolution computation, and L3 in an enabled custom trainer. The subsequent Candidate Review verifies these commitments against the authored trace-local delta.

\subsection{Scoped Candidate Realization and Pre-Execution Validation}
\label{sec:scoped_realization}

Open-ended code generation creates an attribution risk: a candidate can appear superior because it changed preprocessing, evaluation, or shared infrastructure rather than the stated EDA mechanism. HEDGE separates trace-local editable artifacts from protected comparison-critical components. Let $\mathcal{S}_{\mathrm{edit}}$ denote the declared candidate scope and $\mathcal{S}_{\mathrm{prot}}$ the shared framework, task interface, dataset definition, and evaluation logic. A valid candidate delta obeys
\begin{equation}
...
After a candidate has been authored, one role-separated Candidate Reviewer performs a focused pre-execution review. It first verifies that the card names concrete EDA objects, references an EDA analysis rule, states a causal mechanism and falsification condition, and is not merely generic hyperparameter tuning. It then compares the exact candidate delta with its parent. Every claimed L1/L2/L3 mechanism must map to changed evidence in the corresponding trace-local implementation surface; unchanged parent code is context, not evidence for the child. Conversely, every material source or configuration delta must be declared. The review rejects or requests revision for undeclared changes, changed controls, scope violations, or generic parameter tuning presented as an EDA mechanism. Such parameters may appear only as declared controlled ablations attached to a separate current-trace semantic change.

The reviewer writes \texttt{candidate\_review.json} with criterion-level verdicts, implemented and unimplemented claims, unclaimed changes, confounders, semantic-level evidence, and SHA-256 fingerprints of the hypothesis card and implementation. Its reviewer identifier must differ from that of the proposer. Acceptance requires concrete implementation evidence for at least one claim and no unresolved issue fields; any later card, source, or comparison-relevant configuration edit makes the review stale.
...
HEDGE separates a semantic intervention from generic parameter optimization. For a fixed candidate formulation $\mathcal{C}_i$, a task-approved inner search may optimize training parameters $\lambda$:
\begin{equation}
...
The result analyst evaluates whether the child-parent comparison is comparable, whether the validation metric changes in the expected direction, whether predicted task-specific observations occur, and whether the falsification condition is triggered. When the available evidence does not support a mechanism-level judgment, the analyst records the uncertainty and may request a narrowly scoped diagnostic. The analyst then records
\begin{equation}
\begin{aligned}
...
\end{equation}
from which the next parent and hypothesis are selected. The memory consists of structured cards, candidate reviews, context-compatible validation summaries, code diffs, and analysis records---not a raw conversation transcript. A supported mechanism can be extended or ablated; a refuted direction can be pruned; and an inconclusive result can motivate a narrower diagnostic or restart. Thus, the next decision depends on mechanism-level evidence and recorded validation metrics. Held-out evaluation is reserved for final reporting after the search budget is exhausted and does not modify trace memory.
...
In summary, HEDGE converts open-ended LLM exploration into a controlled sequence of EDA modeling decisions. Semantic hypothesis grounding narrows the search toward meaningful representation, computation, and learning-signal mechanisms; scoped trace-local realization and fingerprinted review gates protect evidence attribution; and evidence-grounded trace memory turns completed experiments into reusable constraints for subsequent exploration. Section~\ref{exp} evaluates whether these mechanisms discover useful EDA-native GNN designs beyond fixed-space automation.
