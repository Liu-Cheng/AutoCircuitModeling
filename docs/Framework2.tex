...
Figure~\ref{fig:framework} illustrates one HEDGE research cycle. Given an active context $\kappa$ and completed trace memory $\mathcal{M}_t=\{T_0,\ldots,T_t\}$, a proposer selects a comparable parent, authors one hypothesis card, and creates a trace-local implementation. One independent Candidate Reviewer then evaluates EDA grounding and bidirectional alignment between the card and the exact code/configuration delta. The runtime validates the workspace contracts and executes the candidate. During search, each run selects its checkpoint and produces evidence only on the fixed validation split. The validation evidence is visible to both human and agent researchers for comparison and refinement. A result analyst records the outcome and recommends the next action from this validation-only bundle. After the pre-specified search budget is exhausted, a validation-only rule freezes the top three candidates. Only then are these three frozen candidates each evaluated once on the held-out test split, without test-based re-ranking or further search. The completed trace is appended to $\mathcal{M}_t$ and can be extended, ablated, pruned, or used as the root of a new branch.
...
\begin{algorithm}[t]
\caption{Protocol-Grounded Semantic Exploration in \sysname}
...
\end{equation}
where $\mathcal{G}_i=(M_i,R_i,O_i,A_i,L_i)$ is the semantic grounding of the hypothesis: its task mechanism, modeling rationale, concrete EDA objects, analysis rule, and intended L1--L3 levels. $\mathcal{X}_i=(E_i,F_i,V_i)$ specifies its experimental claim through the expected observation, falsification condition, and controlled variables; $S_i$ declares the permitted implementation scope; and $P_i$ cites the validation evidence inherited from the parent trace. A child card must cite its actual parent. During search, validation evidence is the sole basis for trace comparison and result analysis. Test evidence is unavailable to the proposer, Candidate Reviewer, and result analyst until the validation-only selection rule has frozen the top three candidates.
...
The L1--L3 labels are commitments rather than a closed library of model operations. They state where the claimed mechanism must be realized: L1 in an enabled graph transformation, L2 in model or convolution computation, and L3 in an enabled custom trainer. The subsequent Candidate Review verifies this realization against the actual trace-local delta.

\subsection{Scoped Candidate Realization and Validation}
\label{sec:scoped_realization}

Open-ended code generation creates an attribution risk: a candidate can appear superior because it changed preprocessing, evaluation, seeds, or shared infrastructure rather than the stated EDA mechanism. HEDGE separates trace-local editable artifacts from protected comparison-critical components. Let $\mathcal{S}_{\mathrm{edit}}$ denote the declared candidate scope and $\mathcal{S}_{\mathrm{prot}}$ the shared framework, task interface, dataset definition, and evaluation logic. A valid candidate delta obeys
\begin{equation}
...
After a candidate has been authored, one role-separated Candidate Reviewer performs the focused pre-execution review. It first checks the hypothesis card for concrete EDA objects, an EDA analysis rule, a causal mechanism, and an explicit falsification condition. It then compares the exact current delta with the parent trace. Every claimed L1/L2/L3 mechanism must map to changed evidence in the corresponding trace-local implementation surface; unchanged parent code is context, not evidence for the child. Conversely, every material source or configuration change must be declared. The review rejects or requests revision for undeclared changes, changed controls, scope violations, or generic parameter tuning presented as an EDA mechanism. Generic parameters may appear only as controlled ablations attached to a separate current-trace semantic change and cannot themselves support an \texttt{eda\_grounded} classification.

The reviewer writes \texttt{candidate\_review.json}, including its distinct reviewer identifier, criterion-level verdicts, implemented and unimplemented claims, unclaimed changes, confounders, semantic-level evidence, and SHA-256 fingerprints of the hypothesis card and implementation. Acceptance requires concrete implementation evidence for at least one claim and no unresolved issue fields. Any subsequent card, source, or comparison-relevant configuration edit makes the review stale; the runtime detects this before execution.
...
The result analyst evaluates whether the child-parent comparison is comparable, whether the validation-selected checkpoint changes the validation metric in the expected direction, whether predicted task-specific observations occur, and whether the falsification condition is triggered. When the available evidence does not support a mechanism-level judgment, the analyst records the uncertainty and may request a narrowly scoped diagnostic. The analyst then records
\begin{equation}
